...
- 清一色モードは開始画面のチェックで**鬼モード**にできる（1問5秒制限、時間切れは不正解）
- URL: `/game?mode=speed` / `/game?mode=casual`
- スコア: 清一色 `正解×1000 − 不正解×300 + 正答率`、カジュアル `正解×100 − 不正解×50 + 正答率`、下限0点
- 鬼モードのスコア: 連続正解で獲得点が倍々（1000→2000→4000→8000で頭打ち＝最大8倍）、不正解・時間切れで1000にリセット。`− 不正解×300 + 正答率` は同じ。ランキングは清一色（speed）として登録される
- 格付け: スコアから段位を算出（`src/lib/ranks.ts`、初段〜令和の辻斬り）
