...
### ハマりどころ⑥: 「アプリ内購入とサブスクリプション」セクションが表示されない（2026-08-26 解決）

課金商品3つが `READY_TO_SUBMIT` でも、バージョンページに **本来あるはずの「アプリ内購入とサブスクリプション」セクション自体が表示されない**ことがある（Apple Developer Forums で複数報告済みの既知の不具合）。

- API側にも回避策は無い。`reviewSubmissionItems`（新しい審査提出APIの構成要素）が受け付ける関連は
  `appStoreVersion` / `appEvent` / `appStoreVersionExperiment` などに限られ、**課金商品は対象外**
- **解決策**：バージョンページ右上の「審査用に追加」を押して開く**「提出物の下書き」パネルの方に**、
  新しいサブスクグループがあると自動で「〇〇（グループ名）」というカードが追加される。
  そのカードをクリックすると中の個別サブスクを選択でき、さらに買い切り商品（remove_ads）も
  同じパネルの一覧に候補として出てくる。**バージョンページ上のセクションではなく、この提出パネルの中で選ぶ**
- 全部選び終わると警告（「新しいサブスクリプショングループは〜ともに提出する必要があります」）が消え、
  「審査へ提出」ボタンが押せるようになる

### ハマりどころ⑦: IDFAの申告は「はい」が正解（AdMobを積んでいる場合）

ATTを実装していない＝トラッキングしていない、という理由で「いいえ」にしたくなるが**誤り**。

- Google Mobile Ads SDK は ATT を呼ばなくても **IDFA関連コード（AdSupport.framework）をバイナリにリンクする**
- Apple はバイナリを機械的にスキャンしており、**IDFA関連コードが含まれるのに「いいえ」と申告すると、
  審査以前にバイナリごと恒久的に拒否される**という実例が複数ある
- 正しくは `usesIdfa = true` にした上で、用途は**「App内で広告を配信する」のみ**にチェック
  （インストール追跡・アクション追跡はチェックしない＝実際に行っていないため）
- API上は `PATCH /v1/appStoreVersions/{id}` の `usesIdfa` 属性で設定できるが、
  **用途の内訳チェックボックスはAPIに存在せず、事前に true にしておいても
  提出ウィザードでの追加確認をスキップできる**（2026-08-26 の提出ではIDFAの質問自体が出なかった）
...
- **2026-09-18: v1.2.0 (build 8) アップロード成功**（Delivery UUID c87add85-b510-496a-8871-f5736d892192）。英語対応・符計算モードの初公開・UMP同意・ネイティブのルーティング/セーフエリア修正。タグ `ios-v1.2.0-build8`（コミット 5b4f6a7）。審査提出はまだ
