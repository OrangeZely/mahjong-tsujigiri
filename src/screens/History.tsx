...
export default function HistoryPage() {
  ...
  const handleClear = () => {
    if (!confirm(t("プレイ履歴と最高記録を全て削除します。よろしいですか？"))) return;
    clearHistory();
    setRecords([]);
    setBestScore(0);
  };
    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-900 to-gray-800 p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-6">
          <h1 className="text-3xl font-black text-white">{t("📜 プレイ履歴")}</h1>
          <p className="text-gray-400 text-sm mt-1">{t("これまでの戦績（この端末に保存）")}</p>
        </div>

        {/* 最高段位 */}
        {bestScore > 0 && (
          <div className="bg-yellow-500/10 border border-yellow-500/40 rounded-2xl p-4 mb-6 text-center">
            <div className="text-xs text-yellow-200 mb-1">{t("過去最高段位")}</div>
            <div className="text-3xl font-black text-yellow-400">{getRank(bestScore, locale)}</div>
            <div className="text-gray-400 text-sm mt-1">
              {t("最高スコア:")}<span className="text-yellow-300 font-bold">{bestScore.toLocaleString()}</span>
            </div>
          </div>
        )}
        ...
        {/* 履歴リスト */}
        {!loaded ? (
          <div className="text-center text-gray-400 py-12">{t("読み込み中...")}</div>
        ) : records.length === 0 ? (
          <div className="text-center text-gray-400 py-12">
            {t("まだプレイ履歴がありません。一戦交えてきましょう！")}</div>
        ) : (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-white font-bold">
                      {rec.correctCount}{t("正解 /")}{rec.totalAnswered}{t("問 •")}{rec.accuracy}%
                    </div>
                  ...
                  <div className="text-right">
                    ...
                    <div className="text-orange-400 text-xs font-bold">{getRank(rec.score, locale)}</div>
                  </div>
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-sm font-bold text-gray-300">{t("問題")}{j + 1}</span>
                              </div>
                            ...
                            >
                              ...
                              <div className="text-xs space-y-0.5 text-gray-300">
                                  ...
                                  <span className="font-bold text-green-400">
                                    {problem?.correctDiscards
                                      .map((cd) => {
                                        ...
                                        return tileLabel({ suit, num, id: cd }, locale);
                                      })
                                      .join(" / ")}
                                  </span>
                                </div>
                                <div>
                                  <span className="text-gray-500">{t("あなた:")}</span>
                                  <span className={`font-bold ${d.answer.isCorrect ? "text-green-400" : "text-red-400"}`}>
                                    {isTimeout || !d.answer.discardedTile ? t("時間切れ") : tileLabel(d.answer.discardedTile, locale)}
                                  </span>
                                </div>
                                {problem?.description && (
                                  <div className="mt-2 text-base leading-relaxed text-gray-200 bg-white/10 rounded-lg px-3 py-2">
                                    <ProblemExplanation problem={problem} mode={rec.gameMode} />
                                  </div>
                                )}
                              </div>
                            </div>
