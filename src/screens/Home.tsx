    ...
    <main className="min-h-screen bg-gradient-to-b from-gray-900 via-green-950 to-gray-900 flex flex-col items-center justify-center p-4">
      ...
      >
        ...
        <h1 className="text-4xl md:text-5xl font-black text-white mb-1 tracking-tight">
          {t("麻雀")}</h1>
        <h2 className="text-4xl md:text-5xl font-black text-yellow-400 mb-6 tracking-tight">
          {t("辻斬る！")}</h2>
        ...
        {loaded && (playerName && !editingName ? (
          <div className="flex items-center justify-center gap-2 mb-6 flex-wrap">
            <div className="bg-white/10 rounded-xl px-4 py-2">
              <span className="text-gray-400 text-xs mr-2">{t("雀士")}</span>
            ...
            {bestScore > 0 && (
              <div className="bg-yellow-500/10 border border-yellow-500/40 rounded-xl px-4 py-2">
                <span className="text-yellow-200 text-xs mr-2">{t("最高段位")}</span>
                <span className="text-yellow-400 font-black">{getRank(bestScore, locale)}</span>
              </div>
            )}
          </div>
        ) : (
          <div className="bg-white/10 border border-white/20 rounded-2xl p-4 mb-6">
            <p className="text-white text-sm font-bold mb-3">
              {playerName ? t("プレイヤー名を変更") : t("⚔️ プレイヤー名を登録して参戦！")}
            </p>
            <div className="flex gap-2">
              <input
                type="text"
                placeholder={t("プレイヤー名")}
                value={nameInput}
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <div className="text-white font-black text-xl">{t("清一色モード")}</div>
              <div className="text-red-300 text-xs">{t("染め手特訓・高難易度")}</div>
            </div>
          </div>
          <div className="text-gray-300 text-sm space-y-1">
            <div>{t("・清一色の何切る問題に挑戦")}</div>
            <div>{t("・60秒間で何問正解できるか")}</div>
            <div>{t("・")}<span className="text-yellow-300 font-bold">{t("👹鬼斬りモード")}</span>{t("は1問5秒＆連続正解で獲得点倍々")}</div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <span className="text-3xl">🧘</span>
            <div>
              <div className="text-white font-black text-xl">{t("何切るモード")}</div>
              <div className="text-green-300 text-xs">{t("60秒間で何問正解できるか")}</div>
            </div>
          </div>
          <div className="text-gray-300 text-sm space-y-1">
            <div>{t("・1問ごとの制限時間は")}<span className="text-yellow-300 font-bold">{t("なし")}</span></div>
            <div>{t("・60秒間で解いた問題数を競う")}</div>
            <div>{t("・")}<span className="text-yellow-300 font-bold">{t("👺鬼斬りモード")}</span>{t("は1問5秒＆連続正解で獲得点倍々")}</div>
          </div>
        </motion.div>
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <span className="text-3xl">🧮</span>
            <div>
              <div className="text-white font-black text-xl">{t("符計算モード")}</div>
              <div className="text-purple-300 text-xs">{t("60秒間で符計算を何問解けるか")}</div>
            </div>
          </div>
          <div className="text-gray-300 text-sm space-y-1">
            <div>{t("・完成した手牌を見て合計符を4択で回答")}</div>
            <div>{t("・60秒間で解いた問題数を競う")}</div>
            <div>{t("・")}<span className="text-yellow-300 font-bold">{t("👹鬼斬りモード")}</span>{t("は1問5秒＆連続正解で獲得点倍々")}</div>
          </div>
        </motion.div>
        ...
        >
          {t("⭐ プレミアム（プレイ無制限・広告なし）")}</motion.button>
