...
## Resubmission prep (2026-09-24)

Build 10 was attached to the `1.2.0` version record, which moved it from `REJECTED` back to `PREPARE_FOR_SUBMISSION`. The review notes (`appStoreReviewDetail`) were rewritten to reference the original rejection's Submission ID, explain the Scene-lifecycle crash and its fix, note that a crash-free launch was verified on an iOS 27.0 simulator, and describe what changed in 1.2.0 (English, Fu Practice, the updated free-play/rewarded-ad limit, the ad privacy choices entry point). The three approved in-app purchases needed no resubmission.

Creating the actual `appStoreVersionSubmissions` resource failed with a 403 from this session's API key (`FORBIDDEN_ERROR`, "Allowed operation is: DELETE") even though every other write this session made — including the beta review submission for build 9 earlier — succeeded. No stuck prior submission was found (`include=appStoreVersionSubmission` on the version returned an empty array), so this reads as a role restriction on the key rather than a leftover resource: submitting for review may require the Account Holder's own authority in a way an Admin-scoped API key cannot exercise. The user submits from the App Store Connect website instead; everything else is prepared.
